...
import audioMAE_utils as mae_u
...
def get_audioMAE_embeddings_and_labels(dataloader, augment=False):
    """
...
def process_audio(
    ...
    """Process audio with optional augmentations, returns list of embeddings"""
    # Load and pad original audio
    audio, sr = torchaudio.load(audio_path)
    audio = audio.numpy()[0]  # Use first channel
    
    # Original audio processing
    target_length = total_duration * sr
    audio_padded = np.pad(audio, (0, target_length - len(audio))) if len(audio) < target_length else audio[:target_length]
    original_emb = _process_padded_audio(audio_padded, sr, segment_duration, num_segments, device)
    
    embeddings = [original_emb]

    # Apply augmentations if requested
    if augment and augmentation_strategies:
        for strategy in augmentation_strategies:
            try:
                # Create augmented version
                augmented_audio = apply_augmentation(audio_padded, sr, strategy)
                
                # Re-pad augmented audio to maintain exact duration
                aug_padded = np.pad(augmented_audio, (0, target_length - len(augmented_audio))) if len(augmented_audio) < target_length else augmented_audio[:target_length]
                
                # Process augmented version
                aug_emb = _process_padded_audio(aug_padded, sr, segment_duration, num_segments, device)
    ...
    return embeddings  # List of (30, 512, 768) arrays


def _process_padded_audio(padded_audio, sr, segment_duration, num_segments, device):
    """Helper function to process already-padded audio"""
    ...
    for i in range(num_segments):
        ...
        # Temporary file handling for model compatibility
        with tempfile.NamedTemporaryFile(suffix='.wav') as tmpfile:
            sf.write(tmpfile.name, segment, sr)
            embedding = mae_u.compute_audio_embeddings(
                tmpfile.name,
                audio_inits,
                "audiomae",
                device,
...
def load_features_and_labels(split_name, folder_name="audioMAE", augment=False):
    """
