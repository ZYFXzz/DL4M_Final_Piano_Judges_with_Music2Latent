...
class AudioMAE(nn.Module):
    ...
    def from_pretrained(self, ckpt_path: str) -> str:
        ...
        checkpoint = torch.load('/Users/nicholasboyko/academia/school/deep-learning/DL4M_Final_Piano_Judges_with_Music2Latent/feature_AudioMAE/AudioMAE/finetuned.pth', 
        ...
        if ckpt_path != '/Users/nicholasboyko/academia/school/deep-learning/DL4M_Final_Piano_Judges_with_Music2Latent/feature_AudioMAE/AudioMAE/finetuned.pth': # re-trained case
            checkpoint_ = torch.load(ckpt_path,
                                    map_location=self.device,
                                    weights_only=False)
    ...
    def get_patch_embeddings(self, batch):
        # batch: [B, 1, T, F]
        x = self.model.patch_embed(batch)  # [B, num_patches, embed_dim]
        # Add positional embedding (skip CLS token)
