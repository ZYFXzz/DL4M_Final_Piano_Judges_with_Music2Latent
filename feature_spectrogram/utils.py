...
def filter_single_label_tracks(dataloader):
    ...
    for idx in range(len(dataloader)):
        ...
        if sum(label) == 1:  # Keep only single-labeled tracks
            # Get the row from metadata using the index
            row = dataloader.metadata.iloc[idx]
...
def display_class_examples(dataloader, num_examples_per_class=2):
    """
    Displays mel spectrograms and plays audio for examples from each class.
    
    Parameters:
    dataloader (TechniqueDataloader): The dataloader containing the audio data
    num_examples_per_class (int): Number of examples to show per class
    """
    # Create a dictionary to store examples for each class
    class_examples = {label: [] for label in LABEL_COLUMNS}
    
    # Collect examples for each class
    for idx in range(len(dataloader)):
        data = dataloader[idx]
        label = data["label"]
        track = dataloader.metadata.iloc[idx]
        
        # Add to each class's examples if the track has that label
        for i, label_name in enumerate(LABEL_COLUMNS):
            if label[i] == 1 and len(class_examples[label_name]) < num_examples_per_class:
                class_examples[label_name].append(track)
    
    # Display examples for each class
    for label_name in LABEL_COLUMNS:
        print(f"\n{'='*50}")
        print(f"Examples for class: {label_name}")
        print(f"{'='*50}")
        
        for i, track in enumerate(class_examples[label_name]):
            print(f"\nExample {i+1}:")
